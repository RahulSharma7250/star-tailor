...
const AdminFinishingPage = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchFinishingOrders, 5000);
  ...
  const fetchFinishingOrders = async () => {
    try {
      setLoading(true);
      // Mock API call - GET /api/orders?stage=finishing
      const storedOrders = JSON.parse(localStorage.getItem('orders') || '[]');
      const finishingOrders = storedOrders.filter((order: Order) => 
        order.status === 'blouse_stitched' || 
        order.status === 'dress_stitched' ||
        order.status === 'blouse_finishing_in_progress' ||
        order.status === 'dress_finishing_in_progress'
      );
      setOrders(finishingOrders);
      setError(null);
    } catch (err) {
      setError('Failed to fetch finishing orders');
  ...
  const updateOrderStage = async (orderId: string, newStage: string) => {
    try {
      ...
      const updatedOrders = storedOrders.map((order: Order) =>
        order.orderId === orderId ? { ...order, status: newStage, currentStage: 'finishing' } : order
      );
      localStorage.setItem('orders', JSON.stringify(updatedOrders));
      
      await fetchFinishingOrders();
      ...
      <TableBody>
        {orderList.map((order) => (
          <TableRow key={order.orderId}>
            ...
            <TableCell>
              <Badge 
                variant={order.status.includes('finishing_in_progress') ? 'default' : 'secondary'}
              >
                {order.status.includes('finishing_in_progress') ? 'Finishing in Progress' : 'Ready for Finishing'}
              </Badge>
            </TableCell>
            <TableCell>
            ...
            <TableCell>
              <div className="flex gap-2">
                ...
                {order.status === `${type}_stitched` && (
                  <Button 
                    size="sm" 
                    variant="outline"
                    onClick={() => updateOrderStage(order.orderId, `${type}_finishing_in_progress`)}
                  >
                    Start Finishing
                  </Button>
                )}
                {order.status.includes('finishing_in_progress') && (
                  <Button 
                    size="sm"
                    onClick={() => updateOrderStage(order.orderId, 'ready_for_ironing')}
                  >
                    Mark Complete
                  </Button>
                )}
                {!order.assignedWorker && (
                  <Button 
                    ...
                    onClick={() => assignWorker(order.orderId, `finishing-worker-01`)}
                  >
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
        ...
        <span className="ml-2">Loading finishing orders...</span>
      </div>
    );
  }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Admin - Finishing Section Management
          </CardTitle>
          <div className="flex items-center gap-2">
            <Badge variant="outline">{orders.length} orders in finishing stage</Badge>
          </div>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="blouse" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="blouse">Blouse Finishing ({blouseOrders.length})</TabsTrigger>
              <TabsTrigger value="dress">Dress Finishing ({dressOrders.length})</TabsTrigger>
            </TabsList>
            
            <TabsContent value="blouse" className="mt-6">
              {blouseOrders.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  No blouse orders in finishing stage
                </div>
              ) : (
...
export default AdminFinishingPage;