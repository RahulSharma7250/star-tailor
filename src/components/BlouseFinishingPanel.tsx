...
interface BlouseFinishingPanelProps {
  user: { id: string; name: string; role: string };
}

const BlouseFinishingPanel = ({ user }: BlouseFinishingPanelProps) => {
  ...
  useEffect(() => {
    const loadOrders = () => {
      ...
      // Filter for orders in finishing stage with blouse items
      const blouseFinishingOrders = allOrders.filter((order: Bill) => 
        order.status === 'finishing' && 
        order.items.some(item => item.clothType === 'Blouse')
      );
      
      setOrders(blouseFinishingOrders);
      setFilteredOrders(blouseFinishingOrders);
    };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 items-center">
        ...
        <Badge variant="outline" className="text-sm">
          {filteredOrders.length} Blouse Orders
        </Badge>
      </div>
...
export default BlouseFinishingPanel;