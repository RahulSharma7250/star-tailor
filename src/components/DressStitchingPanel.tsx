...
interface DressStitchingPanelProps {
  user: { id: string; name: string; role: string };
}

const DressStitchingPanel = ({ user }: DressStitchingPanelProps) => {
  ...
  useEffect(() => {
    const loadOrders = () => {
      ...
      // Filter for dress-type orders (Kurti, Jacket) in stitching stage
      const dressOrders = allOrders.filter((order: Bill) => 
        order.status === 'stitching' && 
        order.items.some(item => ['Kurti', 'Jacket'].includes(item.clothType))
      );
      
      setOrders(dressOrders);
      setFilteredOrders(dressOrders);
    };
  ...
  const getDressType = (order: Bill) => {
    const dressItem = order.items.find(item => ['Kurti', 'Jacket'].includes(item.clothType));
    return dressItem?.clothType || 'Dress';
  };

  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Dress Stitching Department</h1>
...
export default DressStitchingPanel;